...
#define CAN_EX_TX_TIMEOUT_MS 2U
#define CAN_EX_TIME_AFTER_EQ(a, b) ((int32_t)((a) - (b)) >= 0)
...
static CAN_HandleTypeDefEx g_can_instances[CAN_EX_MAX_INSTANCES];
static bool g_can_instance_used[CAN_EX_MAX_INSTANCES];
...
CAN_HandleTypeDefEx *CAN_Create(CAN_HandleTypeDef *hcan,
  ...
  CAN_HandleTypeDefEx *hcan_ex = NULL;
  for (uint32_t i = 0U; i < CAN_EX_MAX_INSTANCES; i++) {
    if (!g_can_instance_used[i]) {
      hcan_ex = &g_can_instances[i];
      g_can_instance_used[i] = true;
      break;
    }
  }
  ...
  memset(hcan_ex, 0, sizeof(*hcan_ex));
  hcan_ex->hcan = hcan;
  ...
  default:
    memset(hcan_ex, 0, sizeof(*hcan_ex));
    g_can_instance_used[hcan_ex - g_can_instances] = false;
    return NULL;
  }
...
void CAN_Destroy(CAN_HandleTypeDefEx *hcan) {
  ...
  // 调用平台相关的反初始化函数
  hcan->deinit(hcan);
  if (hcan >= &g_can_instances[0] &&
      hcan < &g_can_instances[CAN_EX_MAX_INSTANCES]) {
    g_can_instance_used[hcan - g_can_instances] = false;
...
static CAN_StatusTypeDef CAN_SendMsg_F4(CAN_HandleTypeDefEx *hcan,
                                        CAN_TxHeaderTypeDef *tx_header,
                                        uint8_t *data, uint32_t *tx_mailbox) {
  if (HAL_CAN_GetTxMailboxesFreeLevel(hcan->hcan) == 0U) {
  ...
  while (HAL_CAN_IsTxMessagePending(hcan->hcan, *tx_mailbox) != 0U) {
    if (CAN_EX_TIME_AFTER_EQ(HAL_GetTick(), start_tick + CAN_EX_TX_TIMEOUT_MS)) {
      HAL_CAN_AbortTxRequest(hcan->hcan, *tx_mailbox);
