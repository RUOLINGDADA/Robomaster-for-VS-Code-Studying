...
#endif
uint8_t sbus_rx_buf[2][18u];
static volatile uint8_t buf_idx = 0;
static volatile uint8_t uart3_error_flag =
    0; // 错误标志（中断只置位，主循环打印）
...
void RemoteControl_Init(void) {
  HAL_UARTEx_ReceiveToIdle_DMA(&huart3, sbus_rx_buf[buf_idx], 18u);
}

static void SBUS_DecodeToRemoteData(const volatile uint8_t *sbus_raw_data,
...
// UART 空闲中断事件回调 DMA的半传输完成也会进这个回调(UART_DMARxHalfCplt)
